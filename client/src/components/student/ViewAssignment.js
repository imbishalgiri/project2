...
class ViewAssignment extends React.Component {
  ...
  handleSubmit = (id, event, index) => {
    ...
    let { faculty, semester, shift, teacherName } =
      this.props.assignmentsFetched.data.assignments[index]._doc;
  ...
  render() {
    ...
    let items = allAssignments.map(
      ({ _doc: { _id, subjectName, teacherName, file, description, status } }, index) => {
        return (
          <tr key={_id}>
            <td>{subjectName}</td>
            <td>
