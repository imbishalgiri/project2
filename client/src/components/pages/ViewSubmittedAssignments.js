...
class ViewSubmittedAssignment extends React.Component {
	...
	handleSubmitGrade = (id, event, index) =>{
		event.preventDefault();
		console.log(id);
		this.setState({[`formDisplay${index}`]: !this.state[`formDisplay${index}`], [`newRemark${index}`]: this.state[`remark${index}`] });
	...
	render() {
		...
		let items = allAssignments.map( ({_id, file, remarks, student:{ firstName, user:{ rollNo }}}, index) => {
			console.log(this.state[`remark${index}`])
			return (
				<tr key={_id}>
					<td>{firstName}</td>
					<td>{rollNo}</td>
					...
					{
						this.state[`formDisplay${index}`] ? <td>
						<form onSubmit={(e) => this.handleSubmitGrade(_id, e, index)}>
							<input
              					 type="text"
              					 className="form-control form-control-lg" 
              					 name={`remark${index}`}
              					 onChange= {this.handleChange}
              					 value={this.state[`remark${index}`]}
              				/>
              				<input type="submit" className="btn btn-info btn-block mt-4" />
						</form>
					</td> : <td onClick={() => this.setState({[`formDisplay${index}`]: !this.state[`formDisplay${index}`], [`remark${index}`]: remarks })}>{this.state[`newRemark${index}`] || remarks}</td>
					}
				    ...
				    <div className="col-md-8 m-auto">
				      ...
				      <form onSubmit={this.handleSubmit} className="row g-3">
				      	...
				      	<div className="col-md-4">
				      		...
				      		<select name="faculty" onChange={this.handleChange} defaultValue={this.state.faculty} className="custom-select mr-sm-2" id="inlineFormCustomSelect">
				      	...
				      	<div className="col-md-4">
				      		...
				      		<select name="sem" onChange={this.handleChange} defaultValue={this.state.semester} className="custom-select mr-sm-2" id="inlineFormCustomSelect">
						        <option value="choose">Choose...</option>
...
export default connect(mapStateToProps, {getSubmittedAssignments, addRemark})(ViewSubmittedAssignment);
